...
public class GithubGravatarGenerator : IGravatarGenerator
{
    ...
    public object GetGravatar(string id)
    {
        ...
        {
            var rec = new RectangleGeometry();
            rec.Rect = new Rect(new Point(i, j), hidden ? new Size() : new Size(1, 1));
    ...
    private Color Hsl2Rgb(double h, double s = 0.7, double b = 0.5)
    {
        ...
        return Color.FromArgb(255, (byte)(arr[hValue % 6] * 255), (byte)(arr[(hValue | 16) % 6] * 255), (byte)(arr[(hValue | 8) % 6] * 255));
    }
}
